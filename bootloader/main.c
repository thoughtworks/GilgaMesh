...
static void turn_led(int index, bool on) {
    if (on) {
...
static void flash_led(int index) {
...
static void flash_leds() {
    for (size_t i = 0; i < 5; i++)
    {
        for (size_t led = LED_START; led <= LED_STOP; led++)
        {
            turn_led(led, true);
        }
        nrf_delay_ms(100);
...
int main(void)
{
    ...
    if (app_reset) {
        // if the dfu is not success, after reboot the bootloader will 
    ...
    else
    {
        ...
        // CAUTION - from the document, should pass false to ble_stack_init() if the device 
        //   is rebooted by a soft reset(calling NVIC_SystemReset()), but on my board, after 
        //   soft reset we still need to reinitialize the ble_stack. so permanently pass true
        //   to ble_stack_init.
        // ble_stack_init(!app_reset);
        ble_stack_init(true);
        scheduler_init();
    ...
    dfu_start = app_reset;
    // dfu_start now only trigger from app_reset 
