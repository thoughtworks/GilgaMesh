...
static bool nfc_is_working = true;
static bool nfc_response_handler_called = true;
...
void nfcEventHandler(uint8_t rx_byte) {
  nfc_response_handler_called = true;
  switch (current_nfc_state) {
    ...
    case GET_PAID_4:
      ...
      if (grab_state == end_grabbing) {
        ...
        } else { // nfc is working, but we did not read a valid badge
          current_nfc_state = NEEDS_RETRY;
        }
      }
      break;
...
void setup_state_machine() {
  ...
  nfc_response_handler_called = false;
}
...
#ifndef NFC_DEBUG
  return NFC_STATUS_DISABLED;
#endif
  return nfc_is_working ? NFC_STATUS_WORKING : NFC_STATUS_ERROR;
}
...
void initialize_uart_nfc() {
  uart_115200_config(NFC_UART_TX, NFC_UART_RX, nfcEventHandler);
  power_down();
  turn_on_nfc_reader();
}
...
void manage_badge_reading(void *data, uint16_t dataLength) {
  ...
  if (get_setup_state() == SETUP_DONE) {
    ...
    if (!nfc_response_handler_called) {
      nfc_is_working = false;
      // power down
    } // else...
